 ...
 * C source code generated on : Thu Oct 18 17:43:17 2018
 *
...
void P2p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
    ...
    /* S-Function Block: P2p2/Heli 3D/HIL Read Encoder Timebase (hil_read_encoder_timebase_block) */
    {
      ...
      if (result < 0) {
        ...
        P2p2_B.HILReadEncoderTimebase_o2 =
          P2p2_DW.HILReadEncoderTimebase_Buffer[1];
  ...
  /* Gain: '<Root>/Gain1' */
  P2p2_B.Gain1 = P2p2_P.Gain1_Gain * P2p2_B.Gain;

  /* Gain: '<S7>/k_pp ' incorporates:
   *  Sum: '<S7>/Sum1'
   */
  P2p2_B.k_pp = (P2p2_B.Joystick_gain_x - P2p2_B.Gain1) * P2p2_P.k_rp;
  if (rtmIsMajorTimeStep(P2p2_M)) {
    /* Constant: '<Root>/Constant1' */
    P2p2_B.Constant1 = P2p2_P.Constant1_Value;

    /* Gain: '<Root>/Gain3' */
    P2p2_B.Gain3 = P2p2_P.Gain3_Gain * P2p2_B.Gain_i;
  }
   ...
   *  Sum: '<S6>/Sum'
   */
  rtb_Sum1_f = (P2p2_B.k_pp - P2p2_B.Gain3) * P2p2_P.k_pp - P2p2_P.Gain4_Gain *
    P2p2_B.Gain_b * P2p2_P.k_pd;
  ...
  rtb_Step = P2p2_X.Integrator_CSTATE;
  if (rtmIsMajorTimeStep(P2p2_M)) {
    /* Sum: '<S3>/Sum' incorporates:
     *  Constant: '<S3>/Constant'
     */
    P2p2_B.Sum = rtb_HILReadEncoderTimebase_o3 + P2p2_P.Constant_Value_p;

    /* Gain: '<S3>/Elevation: Count to rad' */
    P2p2_B.ElevationCounttorad = P2p2_P.ElevationCounttorad_Gain * P2p2_B.Sum;
     ...
     *  Gain: '<Root>/Gain5'
     */
    rtb_DeadZoney = P2p2_P.Constant_Value - P2p2_P.Gain5_Gain * P2p2_B.Gain_e;

    /* Gain: '<S8>/K_ep' */
    P2p2_B.K_ep = P2p2_P.K_ep_Gain * rtb_DeadZoney;
  }
   ...
   *  Sum: '<S8>/Sum'
   */
  rtb_Step1 = ((rtb_Step + P2p2_B.K_ep) - P2p2_P.Gain2_Gain * P2p2_B.Gain_d *
               P2p2_P.K_ed_Gain) + P2p2_B.Constant1;

  /* Gain: '<S1>/Front gain' incorporates:
   *  Sum: '<S1>/Add'
   */
  rtb_Step = (rtb_Sum1_f + rtb_Step1) * P2p2_P.Frontgain_Gain;
  if (rtmIsMajorTimeStep(P2p2_M)) {
    /* Gain: '<S8>/K_ei' */
    P2p2_B.K_ei = P2p2_P.K_ei_Gain * rtb_DeadZoney;
  ...
  if (rtb_Step > P2p2_P.FrontmotorSaturation_UpperSat) {
    P2p2_B.FrontmotorSaturation = P2p2_P.FrontmotorSaturation_UpperSat;
  } else if (rtb_Step < P2p2_P.FrontmotorSaturation_LowerSat) {
    P2p2_B.FrontmotorSaturation = P2p2_P.FrontmotorSaturation_LowerSat;
  } else {
    P2p2_B.FrontmotorSaturation = rtb_Step;
  }
   ...
   *  Sum: '<S1>/Subtract'
   */
  rtb_DeadZoney = (rtb_Step1 - rtb_Sum1_f) * P2p2_P.Backgain_Gain;

  /* Saturate: '<S3>/Back motor: Saturation' */
  if (rtb_DeadZoney > P2p2_P.BackmotorSaturation_UpperSat) {
    P2p2_B.BackmotorSaturation = P2p2_P.BackmotorSaturation_UpperSat;
  } else if (rtb_DeadZoney < P2p2_P.BackmotorSaturation_LowerSat) {
    P2p2_B.BackmotorSaturation = P2p2_P.BackmotorSaturation_LowerSat;
  } else {
    P2p2_B.BackmotorSaturation = rtb_DeadZoney;
  }
  ...
  P2p2_B.TransferFcn = 0.0;
  P2p2_B.TransferFcn += P2p2_P.TransferFcn_C[0] * P2p2_X.TransferFcn_CSTATE[0];
  P2p2_B.TransferFcn += P2p2_P.TransferFcn_C[1] * P2p2_X.TransferFcn_CSTATE[1];

  /* Step: '<S4>/Step' */
  if (P2p2_M->Timing.t[0] < P2p2_P.Step_Time) {
    rtb_Step = P2p2_P.Step_Y0;
  } else {
    rtb_Step = P2p2_P.Step_YFinal;
  }

  /* End of Step: '<S4>/Step' */

  /* Step: '<S4>/Step1' */
  if (P2p2_M->Timing.t[0] < P2p2_P.Step1_Time) {
    rtb_DeadZoney = P2p2_P.Step1_Y0;
  } else {
    rtb_DeadZoney = P2p2_P.Step1_YFinal;
  }

  /* End of Step: '<S4>/Step1' */

  /* Sum: '<S4>/Sum1' */
  P2p2_B.Sum1 = rtb_DeadZoney - rtb_Step;
  if (rtmIsMajorTimeStep(P2p2_M)) {
  }
}
...
void P2p2_derivatives(void)
{
  ...
  _rtXdot->TransferFcn_CSTATE[0U] += P2p2_P.TransferFcn_A[0] *
    P2p2_X.TransferFcn_CSTATE[0];
  _rtXdot->TransferFcn_CSTATE[0U] += P2p2_P.TransferFcn_A[1] *
    P2p2_X.TransferFcn_CSTATE[1];
  _rtXdot->TransferFcn_CSTATE[1] += P2p2_X.TransferFcn_CSTATE[0];
  _rtXdot->TransferFcn_CSTATE[0U] += P2p2_B.TransferFcn;
}
...
void P2p2_initialize(void)
{
  ...
  P2p2_X.TransferFcn_CSTATE[0] = 0.0;
  P2p2_X.TransferFcn_CSTATE[1] = 0.0;
}
...
RT_MODEL_P2p2_T *P2p2(void)
{
  ...
  /* External mode info */
  P2p2_M->Sizes.checksums[0] = (767971228U);
  P2p2_M->Sizes.checksums[1] = (2296431281U);
  P2p2_M->Sizes.checksums[2] = (2271831529U);
  P2p2_M->Sizes.checksums[3] = (3812832891U);
  ...
  {
    P2p2_B.HILReadEncoderTimebase_o1 = 0.0;
    P2p2_B.HILReadEncoderTimebase_o2 = 0.0;
  ...
  /* Initialize Sizes */
  P2p2_M->Sizes.numContStates = (6);   /* Number of continuous states */
  ...
  P2p2_M->Sizes.numBlocks = (72);      /* Number of blocks */
  P2p2_M->Sizes.numBlockIO = (28);     /* Number of block outputs */
  P2p2_M->Sizes.numBlockPrms = (168);  /* Sum of parameter "widths" */
  return P2p2_M;
}
