...
void P4p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P4p2_M)) {
     ...
     */
    P4p2_B.Sum = rtb_DeadZoney + P4p2_P.Constant_Value;
  ...
  /* Gain: '<Root>/Gain1' */
  P4p2_B.Gain1 = P4p2_P.Gain1_Gain * P4p2_B.Gain;
  if (rtmIsMajorTimeStep(P4p2_M)) {
  }

  /* Integrator: '<S2>/Integrator2' */
  for (i = 0; i < 6; i++) {
    P4p2_B.Integrator2[i] = P4p2_X.Integrator2_CSTATE[i];
  }
  ...
  /* Gain: '<Root>/Gain5' */
  P4p2_B.Gain5 = P4p2_P.Gain5_Gain * P4p2_B.Gain_a;
  if (rtmIsMajorTimeStep(P4p2_M)) {
  }

  /* Step: '<S4>/Step1' */
  currentTime = P4p2_M->Timing.t[0];

  /* Step: '<S4>/Step' */
  if (P4p2_M->Timing.t[0] < P4p2_P.Step_Time) {
    unnamed_idx_1 = P4p2_P.Step_Y0;
  } else {
    unnamed_idx_1 = P4p2_P.Step_YFinal;
  }

  /* End of Step: '<S4>/Step' */

  /* Step: '<S4>/Step1' */
  if (currentTime < P4p2_P.Step1_Time) {
    currentTime = P4p2_P.Step1_Y0;
  } else {
    currentTime = P4p2_P.Step1_YFinal;
  }

  /* Sum: '<S4>/Sum1' */
  P4p2_B.Sum1 = unnamed_idx_1 - currentTime;
  if (rtmIsMajorTimeStep(P4p2_M)) {
    /* Gain: '<S10>/Gain' */
    P4p2_B.Gain_d = P4p2_P.Gain_Gain_n * P4p2_B.PitchCounttorad;

    /* Gain: '<Root>/Gain4' */
    P4p2_B.Gain4 = P4p2_P.Gain4_Gain * P4p2_B.Gain_d;
  }

  /* Step: '<S5>/Step' */
  if (P4p2_M->Timing.t[0] < P4p2_P.Step_Time_n) {
    unnamed_idx_1 = P4p2_P.Step_Y0_e;
  } else {
    unnamed_idx_1 = P4p2_P.Step_YFinal_o;
  }

  /* End of Step: '<S5>/Step' */

  /* Sum: '<S5>/Sum' */
  P4p2_B.Sum_o = unnamed_idx_1 - 0.0;
  if (rtmIsMajorTimeStep(P4p2_M)) {
    /* Constant: '<S7>/Constant' */
    P4p2_B.Constant[0] = P4p2_P.P[0];
    P4p2_B.Constant[1] = P4p2_P.P[1];
    P4p2_B.Constant[2] = P4p2_P.P[2];
    P4p2_B.Constant[3] = P4p2_P.P[3];

    /* Constant: '<S7>/Constant1' */
    for (i = 0; i < 6; i++) {
      P4p2_B.Constant1[i] = P4p2_P.K_reg[i];
    }
   ...
   */
  for (i = 0; i < 2; i++) {
    tmp[i] = P4p2_B.Constant1[i + 4] * P4p2_B.Integrator2[3] +
      (P4p2_B.Constant1[i + 2] * P4p2_B.Integrator2[1] + P4p2_B.Constant1[i] *
       P4p2_B.Integrator2[0]);
  }

  /* End of Product: '<S7>/Product2' */

  /* Sum: '<S7>/Sum1' incorporates:
   *  Product: '<S7>/Product'
   *  SignalConversion: '<S7>/TmpSignal ConversionAtProductInport2'
   */
  P4p2_B.Sum1_i[0] = (P4p2_B.Constant[0] * P4p2_B.Sum1 + P4p2_B.Constant[2] *
                      P4p2_B.Sum_o) - tmp[0];
  P4p2_B.Sum1_i[1] = (P4p2_B.Constant[1] * P4p2_B.Sum1 + P4p2_B.Constant[3] *
                      P4p2_B.Sum_o) - tmp[1];
  if (rtmIsMajorTimeStep(P4p2_M)) {
  }

  /* Gain: '<Root>/Gain2' */
  P4p2_B.Gain2 = P4p2_P.Gain2_Gain * P4p2_B.Gain;
  if (rtmIsMajorTimeStep(P4p2_M)) {
  }

  /* Gain: '<Root>/Gain3' */
  P4p2_B.Gain3 = P4p2_P.Gain3_Gain * P4p2_B.Gain_a;
  if (rtmIsMajorTimeStep(P4p2_M)) {
  ...
  /* Sum: '<Root>/Sum' */
  rtb_Frontgain = P4p2_B.Constant1_d + P4p2_B.Sum1_i[0];
  if (rtmIsMajorTimeStep(P4p2_M)) {
    /* Constant: '<S2>/A' */
    memcpy(&P4p2_B.A[0], &P4p2_P.A_sys[0], 36U * sizeof(real_T));

    /* Constant: '<S2>/B' */
    memcpy(&P4p2_B.B[0], &P4p2_P.B_sys[0], 12U * sizeof(real_T));
    for (i = 0; i < 18; i++) {
      /* Constant: '<S2>/C' */
      P4p2_B.C[i] = P4p2_P.C_sys[i];
   ...
   *  Sum: '<S2>/Sum3'
   */
  tmp_2[0] = P4p2_B.Gain_d;
  tmp_2[1] = P4p2_B.Gain_a;
  tmp_2[2] = P4p2_B.Gain_h;
  tmp_2[3] = P4p2_B.Gain;
  tmp_2[4] = P4p2_B.Gain_l;
  tmp_2[5] = P4p2_B.Gain_k;
  for (i = 0; i < 6; i++) {
    /* Product: '<S2>/Bu' incorporates:
     *  Sum: '<S2>/Sum3'
     */
    tmp_0[i] = 0.0;
    tmp_0[i] += P4p2_B.B[i] * P4p2_B.Sum1_i[0];
    tmp_0[i] += P4p2_B.B[i + 6] * P4p2_B.Sum1_i[1];
    ...
    tmp_1[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp_1[i] += P4p2_B.A[6 * i_0 + i] * P4p2_B.Integrator2[i_0];
    }

    /* End of Product: '<S2>/Ax_hat' */
    tmp_3[i] = P4p2_P.Gain9_Gain * tmp_2[i];
  }
  ...
  for (i = 0; i < 3; i++) {
    tmp_4[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp_4[i] += P4p2_B.C[3 * i_0 + i] * tmp_3[i_0];
    }
  }
  ...
  for (i = 0; i < 3; i++) {
    tmp_5[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp_5[i] += P4p2_B.C[3 * i_0 + i] * P4p2_B.Integrator2[i_0];
    }
  }
   ...
   */
  for (i = 0; i < 6; i++) {
    P4p2_B.Sum5[i] = ((P4p2_B.L[i + 6] * unnamed_idx_1 + P4p2_B.L[i] *
                       currentTime) + P4p2_B.L[i + 12] * unnamed_idx_2) +
      (tmp_0[i] + tmp_1[i]);
  }
   ...
   *  Sum: '<S1>/Add'
   */
  currentTime = (P4p2_B.Sum1_i[1] + rtb_Frontgain) * P4p2_P.Frontgain_Gain;

  /* Saturate: '<S3>/Front motor: Saturation' */
  if (currentTime > P4p2_P.FrontmotorSaturation_UpperSat) {
    P4p2_B.FrontmotorSaturation = P4p2_P.FrontmotorSaturation_UpperSat;
  } else if (currentTime < P4p2_P.FrontmotorSaturation_LowerSat) {
    P4p2_B.FrontmotorSaturation = P4p2_P.FrontmotorSaturation_LowerSat;
  } else {
    P4p2_B.FrontmotorSaturation = currentTime;
  }
   ...
   *  Sum: '<S1>/Subtract'
   */
  currentTime = (rtb_Frontgain - P4p2_B.Sum1_i[1]) * P4p2_P.Backgain_Gain;

  /* Saturate: '<S3>/Back motor: Saturation' */
  if (currentTime > P4p2_P.BackmotorSaturation_UpperSat) {
    P4p2_B.BackmotorSaturation = P4p2_P.BackmotorSaturation_UpperSat;
  } else if (currentTime < P4p2_P.BackmotorSaturation_LowerSat) {
    P4p2_B.BackmotorSaturation = P4p2_P.BackmotorSaturation_LowerSat;
  } else {
    P4p2_B.BackmotorSaturation = currentTime;
  }
...
void P4p2_derivatives(void)
{
  ...
  for (i = 0; i < 6; i++) {
    _rtXdot->Integrator2_CSTATE[i] = P4p2_B.Sum5[i];
  }
...
void P4p2_initialize(void)
{
...

  {
    ...
    for (i = 0; i < 6; i++) {
      P4p2_X.Integrator2_CSTATE[i] = P4p2_P.Integrator2_IC;
    }
...
RT_MODEL_P4p2_T *P4p2(void)
{
  ...
  /* External mode info */
  P4p2_M->Sizes.checksums[0] = (1834822835U);
  P4p2_M->Sizes.checksums[1] = (2351954703U);
  P4p2_M->Sizes.checksums[2] = (2786159939U);
  P4p2_M->Sizes.checksums[3] = (140383754U);
...

  {
    ...
    P4p2_B.Gain4 = 0.0;
    P4p2_B.Sum_o = 0.0;
    P4p2_B.Constant[0] = 0.0;
    P4p2_B.Constant[1] = 0.0;
    P4p2_B.Constant[2] = 0.0;
    P4p2_B.Constant[3] = 0.0;
    P4p2_B.Sum1_i[0] = 0.0;
    P4p2_B.Sum1_i[1] = 0.0;
    P4p2_B.Gain2 = 0.0;
    P4p2_B.Gain3 = 0.0;
  ...
  P4p2_M->Sizes.numBlocks = (91);      /* Number of blocks */
  P4p2_M->Sizes.numBlockIO = (37);     /* Number of block outputs */
  P4p2_M->Sizes.numBlockPrms = (253);  /* Sum of parameter "widths" */
  return P4p2_M;
}
