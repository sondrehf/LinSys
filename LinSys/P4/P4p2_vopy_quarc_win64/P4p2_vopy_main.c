...
typedef RT_MODEL_P4p2_vopy_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * P4p2_vopy(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(P4p2_vopy_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(P4p2_vopy_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://P4p2_vopy:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: P4p2_vopy -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(P4p2_vopy_DW.HILReadEncoderTimebase_Task,
          (t_clock) P4p2_vopy_P.HILReadEncoderTimebase_clock, 500.0, -1)
          ;
        if (result == 0) {
